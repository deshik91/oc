<?php
namespace Opencart\Admin\Controller\Extension\Ciaccountsidebar\Module;

class AccountSidebar extends \Opencart\System\Engine\Controller {
	private $error = array();

	public function install(): void {
		$file = DIR_OPENCART . 'extension/ciaccountsidebar/system/config/accountdashboard/account_sidebar.php';
	...
	public function index(): void {
		$this->load->language('extension/ciaccountsidebar/module/account_sidebar');

		$this->document->setTitle($this->language->get('heading_title'));
		$this->document->addStyle(HTTP_CATALOG . '/extension/ciaccountsidebar/admin/view/javascript/ciaccountdashboard/colorpicker/css/colorpicker.css');
  		$this->document->addScript(HTTP_CATALOG . '/extension/ciaccountsidebar/admin/view/javascript/ciaccountdashboard/colorpicker/js/colorpicker.js');

		$this->load->model('setting/setting');

		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('module_account_sidebar', $this->request->post);

			$this->session->data['success'] = $this->language->get('text_success');

			$this->response->redirect($this->url->link('extension/ciaccountsidebar/module/account_sidebar', 'user_token=' . $this->session->data['user_token'] . '&type=module', true));
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/ciaccountsidebar/module/account_sidebar', 'user_token=' . $this->session->data['user_token'], true)
		);
		...
		$data['action'] = $this->url->link('extension/ciaccountsidebar/module/account_sidebar', 'user_token=' . $this->session->data['user_token'], true);
		...
		if ($this->request->server['REQUEST_METHOD'] != 'POST') {
			$module_info = $this->model_setting_setting->getSetting('module_account_sidebar');
		}
		...
		$this->response->setOutput($this->load->view('extension/ciaccountsidebar/module/account_sidebar', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/ciaccountsidebar/module/account_sidebar')) {
		...
		if (isset($this->request->post['module_account_sidebar_title'])) {
			foreach ($this->request->post['module_account_sidebar_title'] as $row => $description) {
				...
				foreach ($description['description'] as $language_id => $value) {
					if ((ci_strlen($value['title']) < 1) || (ci_strlen($value['title']) > 128)) {
				...
				if(isset($description['link_title'])) {
					foreach ($description['link_title'] as $link_title_row => $link_title) {
						foreach ($link_title['module_account_sidebar_link_title_description'] as $language_id => $link_title_description) {
							...
							if ((ci_strlen($link_title_description['name']) < 1) || (ci_strlen($link_title_description['name']) > 128)) {
...
function ci_strlen(string $s): string {
	if(VERSION >= '4.0.1.0') {
		return \Opencart\System\Helper\Utf8\strlen($s);
	} else {
